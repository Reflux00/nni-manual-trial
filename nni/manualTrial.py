...
from .runtime.trial_command_channel import get_default_trial_command_channel, set_default_trial_command_channel
from .runtime.trial_command_channel.v3 import TrialCommandChannelV3
...
_trial_env_from_chanel: Dict|None = None


def get_nni_experimentId_from_port(port):
    trial_info_url = f'http://localhost:{port}/trial-info/experiment-id'
    r = requests.get(trial_info_url)
...
def get_trial_env(sequence_id, port):
    trial_info_url = f'http://localhost:{port}/trial-info/local-env/{sequence_id}'
    r = requests.put(trial_info_url, json={'type':'request_trial_info', 'trialId': f'{sequence_id}', 'exitCode': 0})
    r = requests.get(trial_info_url)
    env = r.json()
...
def get_next_parameter(sequence_id) -> Parameters:
    """
    ...
    return _params['parameters'], _params
