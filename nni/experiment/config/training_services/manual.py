...
class ManualConfig(TrainingServiceConfig):
    platform: Literal['manual'] = 'manual'
    ...
    def _validate_canonical(self):
        ...
        if self.trial_gpu_number and self.use_active_gpu is None:
            raise ValueError(
                'ManualConfig: please set use_active_gpu to True if your system has GUI, '
        ...
        if not self.trial_gpu_number and self.max_trial_number_per_gpu != 1:
            raise ValueError('ManualConfig: max_trial_number_per_gpu does not work without trial_gpu_number')
