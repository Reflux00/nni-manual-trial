...
import { ManualTrialKeeper } from 'common/trial_keeper/manualKeeper';

export class ManualLocalTrainingServiceV3 implements TrainingServiceV3 {
    private config: LocalConfig;
    private env: EnvironmentInfo;
    private log: Logger;
    private trialKeeper: ManualTrialKeeper;
    ...
    constructor(trainingServiceId: string, config: TrainingServiceConfig) {
        ...
        this.trialKeeper = new ManualTrialKeeper(this.env.id, 'local', Boolean(config.trialGpuNumber));
    }
    ...
    public async createTrial(
        ...
        const opts: ManualTrialKeeper.TrialOptions = {
            id: trialId,
    ...
    public async sendParameter(trialId: string, parameter: Parameter): Promise<void> {
        ...
        const command = { type: 'parameter', parameter, trialId};
